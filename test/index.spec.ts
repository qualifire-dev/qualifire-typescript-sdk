...
describe('index', () => {
  ...
  describe('policy_include_tools field', () => {
    it('should pass policy_include_tools in the API request (direct mode)', async () => {
      const capturedBodies: Record<string, unknown>[] = [];
      global.fetch = jest
        .fn()
        .mockImplementation((_url: string, options: { body: string }) => {
      ...
      expect(capturedBodies).toHaveLength(1);
      const body = capturedBodies[0];
      expect(body.policy_include_tools).toBe(true);
    });

    it('should pass policy_include_tools in the API request (framework mode)', async () => {
      const capturedBodies: Record<string, unknown>[] = [];
      global.fetch = jest
        .fn()
        .mockImplementation((_url: string, options: { body: string }) => {
